...
for ((x=0; x < i; x++)); do
	...
	convert "${files[x]}" -channel green -evaluate multiply 0.90 +channel -normalize -auto-level -fill khaki1 -colorize 35% -brightness-contrast -10x10 "$wd"/edited-image/"${files[x]}"
done
